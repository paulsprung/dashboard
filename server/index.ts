...
const rateLimit = (scope: string, max: number, windowMs: number) => (req: express.Request, res: express.Response, next: express.NextFunction) => {
...
app.post('/api/setup/start', rateLimit('setup-start', 20, 15 * 60_000), (req, res) => {
...
app.post('/api/setup/root/registration-options', rateLimit('root-reg-opt', 60, 15 * 60_000), async (req, res) => {
...
app.post('/api/setup/root/verify-registration', rateLimit('root-reg-verify', 60, 15 * 60_000), async (req, res) => {
  if (setupState.completed) return res.status(410).json({ error: 'Root setup registration is disabled after setup completion' });
...
app.post('/api/auth/passkey/registration-options', rateLimit('reg-opt', 60, 15 * 60_000), async (req, res) => {
  const { email, inviteToken } = req.body as { email?: string; inviteToken?: string };
  if (!email) return res.status(400).json({ error: 'Email is required' });
  const normalizedEmail = email.trim().toLowerCase();
...
app.post('/api/auth/passkey/verify-registration', rateLimit('reg-verify', 60, 15 * 60_000), async (req, res) => {
...
app.post('/api/auth/passkey/authentication-options', rateLimit('auth-opt', 120, 15 * 60_000), async (req, res) => {
  const { email, inviteToken } = req.body as { email?: string; inviteToken?: string };
  if (!email) return res.status(400).json({ error: 'Email is required' });
...
app.post('/api/auth/passkey/verify-authentication', rateLimit('auth-verify', 120, 15 * 60_000), async (req, res) => {
  ...
  const shouldUseSecureCookie = isProduction || !getEffectiveOrigin(req).startsWith('http://localhost');
  res.setHeader('Set-Cookie', `${sessionCookieName}=${encodeURIComponent(loginSessionToken)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=86400${shouldUseSecureCookie ? '; Secure' : ''}`);
...
app.post('/api/auth/logout', (req, res) => {
  ...
  void persistState();
  const isSecure = isProduction || !getEffectiveOrigin(req).startsWith('http://localhost');
...
app.post('/api/admin/invites', rateLimit('admin-invite', 50, 15 * 60_000), (req, res) => {
  if (!isAdmin(req)) return res.status(403).json({ error: 'Admin access required' });
