...
3. Generate Prisma client (safe to repeat): `npx prisma generate`
4. Start Postgres: `docker compose up -d db`
5. Run migrations: `npx prisma migrate dev --name init`
6. Start app: `npm run dev`
7. Open `/setup` once to create owner + first passkey.

## Codespaces WebAuthn notes
- WebAuthn requires exact origin/RP matching.
- In `.env`, set:
  - `ORIGIN` to your forwarded HTTPS URL (example: `https://fuzzy-space-giggle-3000.app.github.dev`)
  - `RP_ID` to the host only (example: `fuzzy-space-giggle-3000.app.github.dev`)
